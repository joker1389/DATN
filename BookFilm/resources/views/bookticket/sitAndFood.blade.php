                                        ...
                                        <div class="sits__row">
                                            @for($j = 1 ; $j <= $item->Column; $j++)
                                                @if(in_array($CharRow[$i] . $j, $arr_sit))
                                                    <span class="sits__place sits-price--cheap sits-state--not" data-place="{{ $CharRow[$i] . $j }}" data-price='{{ $item->Price }}'>{{ $CharRow[$i] . $j }}</span>
                                                @else
                                                    <span class="sits__place sits-price--cheap" data-place="{{ $CharRow[$i] . $j }}" data-price='{{ $item->Price }}'>{{ $CharRow[$i] . $j }}</span>
                                                @endif
                                        ...
                                        <div class="sits__row">
                                            @for($j = 1 ; $j <= $item->Column; $j++)
                                                @if(in_array($CharRow[$i+$k-1] . $j, $arr_sit))
                                                    <span class="sits__place sits-price--middle sits-state--not" data-place="{{ $CharRow[$i+$k-1] . $j }}" data-price='{{ $item->Price }}'>{{ $CharRow[$i+$k-1] . $j }}</span>
                                                @else
                                                    <span class="sits__place sits-price--middle" data-place="{{ $CharRow[$i+$k-1] . $j }}" data-price='{{ $item->Price }}'>{{ $CharRow[$i+$k-1] . $j }}</span>
                                                @endif
                                        ...
                                        <div class="sits__row">
                                            @for($j = 1 ; $j <= $item->Column; $j++)
                                                @if(in_array($CharRow[$i+$k+$m-1] . $j, $arr_sit))
                                                    <span class="sits__place sits-price--expensive sits-state--not" data-place="{{ $CharRow[$i+$k+$m-1] . $j }}" data-price='{{ $item->Price }}'>{{ $CharRow[$i+$k+$m-1] . $j }}</span>
                                                @else
                                                    <span class="sits__place sits-price--expensive" data-place="{{ $CharRow[$i+$k+$m-1] . $j }}" data-price='{{ $item->Price }}'>{{ $CharRow[$i+$k+$m-1] . $j }}</span>
                                                @endif
            ...
            <form id='film-and-time' class="booking-form" method='post' action='/book/checkout'>
                ...
                <div class="col-sm-6">
                    ...
                    <div class="col-sm-7 col-md-9">
                        ...
                        <p>
                            <button type="button" class="btn btn-secondary btnDown" data-id="{{ $item->ID }}">-</button>
                           <input type="number" name="Quantity[]" id="Quantity_{{ $item->ID }}" min=0 value="0" class="search__field text-center" style="width: 20%; display: initial;">
                           <button type="button" class="btn btn-secondary btnUp" data-id="{{ $item->ID }}">+</button>
                       </p>
