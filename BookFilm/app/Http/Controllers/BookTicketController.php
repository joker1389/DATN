...
class BookTicketController extends Controller
{
    //
     public function BookDateTime($ID){
     	if(Session::get('user') == null){
     		Session::flash('error', 'Bạn vui lòng đăng nhập để đặt vé.');
    ...
    public function BookFoodDrink(Request $request){
        ...
        $ticket = DB::table('room_detail')
                        ->where('ID', 1)
                        ->first();
    	$TotalMoney = $bookSit[$User_ID]['TotalPrice'] + $bookSit[$User_ID]['CountTicket'] * ($ticket->Price * 1000) + $TotalFoodDrink;
    	...
    	if($isPayment == true){
    		...
    		if($food_drink != null){
             ...
             $ticket = DB::table('room_detail')
                        ->where('ID', 1)
                        ->first();
    		$TotalMoney = $booksit[$user_id]['TotalPrice'] + $booksit[$user_id]['CountTicket'] * $ticket->Price + $TotalFoodDrink;
    		$film = DB::table('film')->where('ID', $ID)->select('*')->first();
